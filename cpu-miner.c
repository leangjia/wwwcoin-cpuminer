...
static char	coinbase_address[COINBASE_ADDR_LEN];
...
start:
	...
	if (have_gbt) {
	   req = malloc(strlen(gbt_req) + COINBASE_ADDR_LEN + 1);
	   if(!req)
		return false;
	   sprintf(req, gbt_req,coinbase_address);
	}
...
start:
	...
	while (1) {
		...
		if (have_gbt) {
			req = malloc(strlen(gbt_lp_req) + strlen(lp_id) + COINBASE_ADDR_LEN + 1);
			if(!req)
				goto out;
			sprintf(req, gbt_lp_req, lp_id ,coinbase_address);
		}
...
static void parse_arg(int key, char *arg, char *pname)
{
	...
	case 1013:			/* --coinbase-addr */
		...
		snprintf(coinbase_address, COINBASE_ADDR_LEN, "%s", arg);
		break;
